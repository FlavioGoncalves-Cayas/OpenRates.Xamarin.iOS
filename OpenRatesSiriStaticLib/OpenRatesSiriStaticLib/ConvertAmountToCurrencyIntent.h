...
@property (readwrite, copy, nullable, nonatomic) NSNumber *amount;

@end

@class ConvertAmountToCurrencyIntentResponse;
@class ConvertAmountToCurrencyBaseCurrencyResolutionResult;
...
*/
- (void)resolveBaseCurrencyForConvertAmountToCurrency:(ConvertAmountToCurrencyIntent *)intent withCompletion:(void (^)(ConvertAmountToCurrencyBaseCurrencyResolutionResult *resolutionResult))completion NS_SWIFT_NAME(resolveBaseCurrency(for:with:)) API_AVAILABLE(ios(13.0), watchos(6.0));
 ...
 */
+ (instancetype)successIntentResponseWithBaseAmount:(NSNumber *)baseAmount baseCurrency:(Currency)baseCurrency amount:(INCurrencyAmount *)amount NS_SWIFT_NAME(success(baseAmount:baseCurrency:amount:));

@property (readwrite, assign, nonatomic) Currency baseCurrency;
@property (readwrite, copy, nullable, nonatomic) NSNumber *baseAmount;
...
typedef NS_ENUM(NSInteger, ConvertAmountToCurrencyBaseCurrencyUnsupportedReason) {
